...
def test_027_is_additive_and_keeps_live_text_array_tokens() -> None:
    sql = (Path(__file__).parents[1] / "migrations" / "027_seller_stores.sql").read_text(
        encoding="utf-8"
    )
    assert "CREATE TABLE IF NOT EXISTS commerce_seller_stores" in sql
    assert "ADD COLUMN IF NOT EXISTS" in sql
    assert "serviceability_tokens TEXT[]" in sql
